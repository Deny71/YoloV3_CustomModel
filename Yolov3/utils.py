...
def Load_Yolo_model(type):
    ...
    if type == 1 :
        weights = f"./CustomModel/yolov3_custom"
        yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE_CUSTOM, CLASSES=TRAIN_CLASSES)
        yolo.load_weights(weights)  # use custom weights
    else :
        weights = f"./OriginalModel/yolov3_original"
        yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE_ORIGINAL, CLASSES=TRAIN_CLASSES)
        yolo.load_weights(weights)  # use custom weights
